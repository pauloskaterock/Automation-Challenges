...
def processar_notas_simples(notas):
    ...
    for nota in notas:
        if nota.valor == 0:
            ...
            banco_para_inserir.append(nota)
        else:
    ...
    # Email de cada nota > 5000
    for nota in emails_para_enviar:
        print(f"Email: Nota {nota.numero} - R$ {nota.valor}")

    # Email de notas zeradas
    print(f"Email: Quantidade de notas zeradas = {contador_zeradas}")

    # === PASSO 2: INSERE NO BANCO ===

    for nota in banco_para_inserir:
        print(f"Banco: Inserindo nota {nota.numero} - R$ {nota.valor}")
